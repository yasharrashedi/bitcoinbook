...
((("bitcoin", "defined", id="GSdefine01")))بیتکوین مجموعه‌ای از مفاهیم و تکنولوژی‌ها است که مبنای یک اکوسیستم پول دیجیتال را تشکیل می دهد. واحدهای این ارز که بیتکوین نام دارند برای ذخیره و انتقال ارزش در بین مشارکت‌کنندگان  شبکه بیتکوین مورد استفاده قرار می‌گیرد. کاربران بیتکوین با یکدیگر توسط پرتکل بیتکوین و عموما از طریق اینترنت ارتباط برقرار می‌کنند، اگرچه سایر شبکه‌های انتقال نیز قابل استفاده است. مجموعه پروتکل بیتکوین که به صورت نرم‌افزار متن‌باز در دسترس است، امکان اجرا بر روی بازه وسیعی از دستگاه‌های رایانه‌ای، شامل لپتاپ‌ها و تلفن‌های هوشمند را دارد که در دسترس قرار گرفتن این تکنولوژی را به سادگی  امکانپذیر می ‌نماید.

کاربران می‌توانند بیتکوین را در شبکه برای انجام تقریباً هر چیزی که با پول‌های مرسوم می‌توان انجام داد، منتقل نمایند. از این موارد می‌توان به خرید و فروش کالا،‌ارسال پول به افراد و سازمان ها یا افزایش اعتبار اشاره کرد. بیتکوین امکان خریداری شدن،‌فروخته شدن یا مبادله شدن با سایر ارزها را در صرافی‌های مخصوصی دارد. به تعبیری بیتکوین، یک شکل کامل پول برای اینترنت است به این علت که سریع، امن و بدون مرز است.

بر خلاف ارزهای سنتی، بیتکوین کاملاً مجازی است. هیچ سکه فیزیکی یا حتی سکه دیجیتال به خودی خود وجود ندارد. این سکه‌ها در تراکنش‌هایی که ارزش را از فرستنده به گیرنده منتقل می‌کنند دلالت دارند. کاربران بیتکوین، کلیدهایی دارند که به آن‌ها اجازه می‌دهد، مالکیت بیتکوین‌هایشان را روی شبکه اثبات کنند. این افراد با این کلیدها می‌توانند تراکنشهایی که مقداری از ارزش [که خودشان مالک آن هستند] را آزاد کرده و به واسطه انتقال این ارزش به مالک جدید خرج می‌شود را امضا کنند. این کلیدها معمولاً در یک کیف پول دیجیتالی که روی کامپیوتر یا تلفن هوشمند هر کاربر نصب شده است نگهداری می‌شود. فقط داشتن مالکیت این کلید پیش‌نیاز خرج کردن بیتکوین است که باعث می‌شود کنترل کامل آن در اختیار هر کاربر باشد.

بیتکوین یک سیستم توزیع شده و همتا به همتا است. به طوری که هیچ سرور «مرکزی» یا نقطه کنترل مرکزی وجود ندارد. سکه‌های بیتکوین به واسطه عملیاتی به نام «ماینینگ یا معدن کاوی» که مستلزم رقابت برای یافتن راه‌حل یک مسأله ریاضی در هنگام پردازش تراکنش‌های شبکه بیتکوین است به وجود می‌آیند. هر مشارکت کننده در شبکه بیتکوین (مثلاً هر شخصی که یک دستگاه که پروتکل کامل بیتکوین را اجرا می‌کند را استفاده می‌کند) می‌تواند به عنوان یک ماینر فعالیت داشته باشد که در آن از قدرت پردازشی کامپیوتر خود برای شناسایی و تأیید و ثبت تراکنش ها استفاده می‌کند. به طور متوسط در هر ۱۰ دقیقه،‌یکی از ماینرهای فعال شبکه بیتکوین قادر خواهد بود که تراکنش‌های ۱۰ دقیقه گذشته موجود در شبکه را تأیید نماید که در نتیجه مقداری بیتکوین به عنوان دستمزد جایزه خواهد گرفت.
در‌واقع، ماینینگ بیتکوین عملیات انتشار یا صدور ارز و نقل و انتقالاتی که یک بانک مرکزی انجام می‌دهد را غیرمتمرکز کرده و نیاز به بانک‌های مرکزی را از بین می‌برد.

در پروتکل بیتکوین، الگوریتم‌هایی تعبیه شده است که عملیات ماینینگ را در سراسر شبکه تنظیم می‌کند. سختی پردازشی که ماینرها باید انجام دهند به صورت خودکار و پویا تنظیم می‌گردد، بنابراین به صورت متوسط، در هر ۱۰ دقیقه، بدون درنظر گرفتن تعداد (و قدرت پردازشی) ماینرهایی که در شبکه در حال رقابت با یکدیگر هستند، یک ماینر موفق خواهد شد. پروتکل همچنین میزان بیتکوین ایجاد شده را در هر ۴ سال نصف می‌کند، و تعداد کل بیتکوینی که قرار است تولید شود را به عدد ثابتی نزدیک به ۲۱ میلیون سکه محدود می‌نماید. در نتیجه تعداد بیتکوین در گردش، یک روند قابل پیشبینی که در سال ۲۱۴۰ به عدد ۲۱ میلیون خواهد رسید را دنبال می‌کند. به دلیل روند کاهشی انتشار بیتکوین، در درازمدت این ارز تورم منفی خواهد داشت. به تعبیری دیگر، بیتکوین به علت عدم امکان «چاپ» پول جدید بیشتر از میزان انتشار مورد انتظار، تورم نخواهد داشت.
